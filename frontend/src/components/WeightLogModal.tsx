    ...
    const handleSave = async () => {
        ...
        try {
            ...
            toast({
                variant: "destructive",
                title: t('error'),
                description: t('weightModal.error'),
                duration: 5000
            });
        } finally {
            setSaving(false);
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className={`fixed bottom-0 left-0 right-0 z-50 rounded-t-[28px] bg-white p-6 pb-10 ${isAr ? 'rtl text-right' : 'ltr text-left'}`}
                    ...
                    >
                        ...
                        <div className="mb-8 text-center">
                            <h2 className="font-display text-xl font-bold text-foreground">{t('weightModal.title')}</h2>
                            <p className="mt-1 text-sm text-muted-foreground">{t('weightModal.hint')}</p>
                        </div>
                                ...
                                <div className="text-center min-w-[140px]">
                                    {isEditing ? (
                                        ...
                                        <button
                                            ...
                                            title={t('weightModal.tapToType')}
                                        >
                                            {weight.toFixed(1)}
                                        </button>
                                    )}
                                    <div className="mt-1 text-xs font-medium text-muted-foreground">{t('weightModal.tapToType')}</div>
                                </div>
                        ...
                        <div className="mb-8">
                            <textarea
                                placeholder={t('weightModal.placeholder')}
                                value={note}
                                onChange={(e) => setNote(e.target.value)}
                                className="w-full resize-none rounded-2xl bg-gray-50 p-4 text-sm outline-none ring-2 ring-transparent focus:bg-white focus:ring-[#F5C518]"
                                rows={2}
                            />
                        </div>
                        ...
                        <div className="flex flex-col gap-3">
                            ...
                            >
                                {saving ? <Loader2 className="h-5 w-5 animate-spin" /> : t('weightModal.saveEntry')}
                            </button>
                            <button
                                onClick={onClose}
                                disabled={saving}
